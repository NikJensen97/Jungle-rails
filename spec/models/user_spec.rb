...
RSpec.describe User, type: :model do
  ...
  context "password don't match" do
    user = User.new(email: 'test@test.com', password: 'test', password_confirmation: 'testyyy')
    user.valid?
  ...
  it 'email must be unique' do
    ...
    u = User.new(email: 'test@test.com', password: 'testtest', password_confirmation: 'testtest')
    u.save
  ...
  it 'password length less than 5 characters is invalid' do
    user = User.new(email: 'test@test', password: '1234')
  ...
  it 'password length must be at-least 5 characters' do
    user = User.new(email: 'test@test', password: '12345')
